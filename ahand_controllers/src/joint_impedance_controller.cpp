#include "ahand_controllers/joint_impedance_controller.h"
#include <algorithm>
#include <memory>
#include <functional>
...
ahand_controllers::JointImpedanceController::JointImpedanceController(){}


bool ahand_controllers::JointImpedanceController::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &nh){

    n_joints_ = robot->getNames().size();
    ROS_INFO("Initisalising JointImpedanceController");
    ROS_INFO_STREAM("JointImpedanceController number of joints: "  << n_joints_);
    ...
    dynamic_server_gains_dp_param_->setCallback(boost::bind(&ahand_controllers::JointImpedanceController::gains_pd_callback, this, _1, _2));
    ROS_INFO("Sucessfuly initialised JointImpedanceController");
    return true;
}


void ahand_controllers::JointImpedanceController::starting(const ros::Time& time){
    // read
    ROS_INFO("JointImpedanceController starting");
...
void ahand_controllers::JointImpedanceController::update(const ros::Time& time, const ros::Duration& period){
    // read
    for(size_t i=0; i<joint_handles_.size(); i++) {
        joint_msr_position_[i] = joint_handles_[i].getPosition();
        joint_msr_velocity_[i] = joint_handles_[i].getVelocity();
...
void ahand_controllers::JointImpedanceController::gains_pd_callback(ahand_controllers::gains_pd_paramConfig& config, uint32_t level){
...
PLUGINLIB_EXPORT_CLASS(ahand_controllers::JointImpedanceController, controller_interface::ControllerBase)

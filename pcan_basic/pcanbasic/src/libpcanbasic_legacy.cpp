...
TPCANStatus CAN_GetValue(TPCANHandle Channel, TPCANParameter Parameter,
	...
	try {
		...
		case PCAN_API_VERSION:
			...
			strncpy((char*) Buffer, API_VERSION, strlen(API_VERSION) + 1
					<= BufferLength ? strlen(API_VERSION) + 1 : BufferLength);
			goto leave;
		...
		case PCAN_LOG_LOCATION:
			...
			if (nLogPathStatus != -1)
				strncpy((char*) Buffer, szLogLocationPath, strlen(
						szLogLocationPath) + 1 <= BufferLength ? strlen(
						szLogLocationPath) + 1 : BufferLength);
			else
				strncpy((char*) Buffer, "", 1 <= BufferLength ? 1
						: BufferLength);
			goto leave;
		...
		case PCAN_CHANNEL_VERSION:
			...
			*((char*) Buffer) = 0;
			if (Diag.szVersionString != NULL)
				strncpy((char*) Buffer, Diag.szVersionString, strlen(
						Diag.szVersionString) + 1 <= BufferLength ? strlen(
						Diag.szVersionString) + 1 : BufferLength);
			break;
...
TPCANStatus CAN_SetValue(TPCANHandle Channel, TPCANParameter Parameter,
	...
	try {
		...
		case PCAN_LOG_LOCATION:
			...
			strncpy(szLogLocationPath, (char*) Buffer, strlen((char*) Buffer)
					+ 1 <= LOCAL_STRING_LEN ? strlen((char*) Buffer) + 1
					: LOCAL_STRING_LEN);

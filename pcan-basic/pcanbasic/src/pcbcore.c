 ...
 */
#include "libpcanfd/pcanfd.h"	/* force local header */
#include "libpcanfd/libpcanfd.h"
...
TPCANStatus pcanbasic_read_common(
	...
	case PCANFD_TYPE_STATUS:
		if (pchan->busoff_reset && (msg.flags & PCANFD_ERROR_BUS) == PCANFD_ERROR_BUSOFF) {
			/* auto-reset */
...
TPCANHandle pcanbasic_get_handle(char * device, struct pcaninfo_list * plist) {
	...
	for (i = 0; i < pcil->length; i++) {
		...
		if (strcmp(pci->dev, device) == 0)
		{
			...
			case PCANINFO_HW_DNG:
				return PCAN_DNGBUS1 - 1 + hw_count[pci->hwcategory];
			case PCANINFO_HW_ISA:
				return PCAN_ISABUS1 - 1 + hw_count[pci->hwcategory];
			case PCANINFO_HW_PCC:
				return PCAN_PCCBUS1 - 1 + hw_count[pci->hwcategory];
			case PCANINFO_HW_PCI:
				if (hw_count[pci->hwcategory] < 9)
					return PCAN_PCIBUS1 - 1 + hw_count[pci->hwcategory];
				else
					return PCAN_PCIBUS9 - 1 + hw_count[pci->hwcategory];
			case PCANINFO_HW_USB:
				if (hw_count[pci->hwcategory] < 9)
					return PCAN_USBBUS1 - 1 + hw_count[pci->hwcategory];
				else
					return PCAN_USBBUS9 - 1 + hw_count[pci->hwcategory];
			case PCANINFO_HW_LAN:
				return PCAN_LANBUS1 - 1 + hw_count[pci->hwcategory];

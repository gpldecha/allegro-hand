...
#define CURRENT_RELEASE "Release_20150617_r"
...
#ifdef RTAI
	reading_task = rt_task_init_schmod(nam2num("RDTSK"), 1, 0, 0,
							SCHED_FIFO, 0xF);
...
#ifdef RTAI
	errno = pthread_create(&reading_thr, NULL,
					(void *)reading_task_proc, NULL);
	if (errno) {
		printf("receivetest: Failed to create and start rt task, "
				"code %d\n",errno);
		return errno;
	}
#endif
